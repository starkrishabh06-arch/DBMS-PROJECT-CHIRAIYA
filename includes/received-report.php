...
<head>
  ...
  <title>ExpenseHeist – Received Report</title>
...
$(document).ready(function(){
  ...
  $('#search-input').on('keyup',function(){
    ...
    var found=false;
    if(value){
      $('table tbody tr').filter(function(){var m=$(this).text().toLowerCase().indexOf(value)>-1;$(this).toggle(m);if(m)found=true;});
...
function printReport(){
  ...
  nw.document.write('<style>body{font-family:Arial,sans-serif;color:#111;} table{border-collapse:collapse;width:100%;} td,th{border:1px solid #333;padding:6px 10px;} thead th{background:#1a4a2e;color:#fff;}</style>');
    ...
    <div class="card">
      <div class="card-header">
        <div class="row align-items-center">
          <div class="col-md-6">
            <h4 class="card-title"><i class="bx bx-check-shield" style="color:#4eca7f;margin-right:8px;"></i>Received Report</h4>
          </div>
          <div class="col-md-6 text-right">
            <button class="btn btn-primary" onclick="printReport()"><i class="bx bx-printer"></i> Print Report</button>
          </div>
        </div>
      </div>
      <div class="card-body" id="printable">
        <div class="report-heading">
          <span class="report-label">Received <?= ucfirst($rtype) ?> Report</span>
          <span class="report-range"><i class="bx bx-calendar"></i> <?= $fdate ?> &nbsp;<i class="bx bx-right-arrow-alt"></i>&nbsp; <?= $tdate ?></span>
        </div>
        ...
        <table class="table table-bordered">
          <thead>
            <tr>
              <th>#</th><th>Name</th><th>Date of Lending</th><th>Status</th><th>Description</th><th><?= ucfirst($rtype) ?> Amount</th>
            </tr>
          </thead>
          <tbody>
            <?php $cnt=1; $totalsexp=0;
            while ($row=mysqli_fetch_array($ret)) { ?>
            <tr>
              <td><?= $cnt ?></td>
              <td><?= $row['name'] ?></td>
              <td><?= $row['date_of_lending'] ?></td>
              <td><?php if($row["status"]=="received"){echo '<span class="badge bg-success text-white">Received</span>';}else{echo '<span class="badge bg-warning text-white">Pending</span>';} ?></td>
        ...
        <?php } else { echo "<p class='no-data'><i class='bx bx-info-circle'></i> No received intel for this date range.</p>"; } ?>
      </div>
    </div>
  </div>
</div>
